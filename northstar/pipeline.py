...
EXPERIMENTS = ROOT / "data" / "experiments.json"
...
def new_paper_portfolio(
    ...
    signal = observed_signal(latest, selected, market)
    return {
        ...
        "pending": signal,
        "signal_log": [dict(signal)],
        "daily_snapshots": [],
    }


def observed_signal(latest, selected, market, now=None):
    now = pd.Timestamp(now or datetime.now(timezone.utc))
    calendar = mcal.get_calendar("NYSE").schedule(
        start_date=now.date(), end_date=(now + pd.Timedelta(days=14)).date()
    )
    upcoming = calendar[calendar["market_open"] > now]
    return {
        "signal_session": latest.date().isoformat(),
        "generated_at": now.isoformat(),
        "execute_on": upcoming.index[0].date().isoformat(),
        "market_active": market["active"], "selected": selected,
    }


def advance_observed(state, frames, latest, selected, market, now=None):
    # Legacy signals have no observation timestamp and cannot justify past fills.
    if "generated_at" not in state["pending"]:
        state["pending"] = observed_signal(latest, selected, market, now)
        state.setdefault("signal_log", []).append(dict(state["pending"]))
    last = pd.Timestamp(state["last_processed_session"])
    inception = pd.Timestamp(state.get("started_at", state["last_processed_session"]))
    for session in frames[BENCHMARK].index:
        if last < session <= latest and session >= inception:
            process_paper_session(state, session, frames)
    if state["pending"]["signal_session"] != latest.date().isoformat():
        state["pending"] = observed_signal(latest, selected, market, now)
        state.setdefault("signal_log", []).append(dict(state["pending"]))
    return state
...
def process_paper_session(
    ...
    warnings: list[str] = []
    ledger = state.setdefault("stock_ledger", {})
    for ticker, position in positions.items():
        ledger.setdefault(ticker, {"purchases": position["cost_basis"], "sales": 0.0, "dividends": 0.0, "fees": 0.0, "complete": False})
    pending = state.get("pending", {"selected": [], "signal_session": None})
    execute = pending.get("execute_on") == session.date().isoformat()
    if "execute_on" not in pending:
        execute = False
    if execute and pd.Timestamp(pending["signal_session"]) >= session:
        raise ValueError("A signal cannot trade before its completed session")
    if execute:
        schedule = mcal.get_calendar("NYSE").schedule(start_date=session, end_date=session)
        if schedule.empty or pd.Timestamp(pending["generated_at"]) >= schedule.iloc[0]["market_open"]:
            raise ValueError("Signal was not available before the execution open")
        if session_price(frames[BENCHMARK], session, "open") is None:
            raise ValueError("Benchmark open unavailable; cannot start a matched comparison")
    ...
    for ticker, position in positions.items():
        ...
        if split > 0:
            ...
            position["last_price"] = position.get("last_price", 0.0) / split
        dividend = float(frame.loc[session, "dividends"])
        if dividend > 0:
            state["cash"] += position["shares"] * dividend
            ledger.setdefault(ticker, {"purchases": position["cost_basis"], "sales": 0.0, "dividends": 0.0, "fees": 0.0, "complete": False})["dividends"] += position["shares"] * dividend

    targets = {item["ticker"]: item for item in pending.get("selected", [])} if execute else {}
    ...
    for ticker, position in list(positions.items()):
        ...
        if current_value > desired_value:
            ...
            account = ledger[ticker]
            account["sales"] += sale_value
            account["fees"] += fee
    ...
    for ticker, target in targets.items():
        ...
        account = ledger.setdefault(ticker, {"purchases": 0.0, "sales": 0.0, "dividends": 0.0, "fees": 0.0, "complete": True})
        account["purchases"] += purchase_value
        account["fees"] += fee
        if "reference_price" not in account:
            account.update(reference_price=price, growth=1.0, entry_session=session.date().isoformat(), benchmark_growth=1.0,
                           benchmark_price=session_price(frames[BENCHMARK], session, "open"))
        if position:
    ...
    if benchmark["shares"] == 0 and spy_open and execute:
        purchase = benchmark["cash"] / (1 + cost_rate)
        benchmark["shares"] = purchase / spy_open
        benchmark["cash"] -= purchase * (1 + cost_rate)
    if benchmark["shares"] and spy_close is None:
        raise ValueError("Benchmark close unavailable; refusing an invalid valuation")
    benchmark_value = benchmark["cash"] + benchmark["shares"] * float(spy_close or 0.0)

    stock_records = []
    for ticker, account in ledger.items():
        frame = frames.get(ticker)
        close = session_price(frame, session, "close") if frame is not None else None
        if close is None:
            account["reference_incomplete"] = True
        if close is not None and account.get("reference_price"):
            first = account["entry_session"] == session.date().isoformat()
            split = 1.0
            # Split ratios may be below one for reverse splits.
            if not first:
                split = float(frame.loc[session, "splits"]) or 1.0
            dividend = float(frame.loc[session, "dividends"]) if not first else 0.0
            account["growth"] *= split * (close + dividend) / account["reference_price"]
            account["reference_price"] = close
            spy_div = float(frames[BENCHMARK].loc[session, "dividends"]) if not first else 0.0
            spy_split = (float(frames[BENCHMARK].loc[session, "splits"]) or 1.0) if not first else 1.0
            if spy_close and account.get("benchmark_price"):
                account["benchmark_growth"] *= spy_split * (spy_close + spy_div) / account["benchmark_price"]
                account["benchmark_price"] = spy_close
        value = next((h["market_value"] for h in holdings if h["ticker"] == ticker), 0.0)
        stock_records.append({"ticker": ticker, "profit": clean_number(value + account["sales"] + account["dividends"] - account["purchases"] - account["fees"]),
                              "growth": clean_number(account.get("growth")) if close and not account.get("reference_incomplete") else None,
                              "benchmark_growth": clean_number(account.get("benchmark_growth")) if close and not account.get("reference_incomplete") else None,
                              "entry_session": account.get("entry_session"), "complete": account["complete"]})
...
def update_paper_portfolio(
    ...
    advance_observed(state, frames, latest, latest_selected, latest_market)
    write_json(PAPER_PORTFOLIO, state)
    return state
...
def update_experiments(frames, latest, screen, market):
    from .experiments import VARIANTS, metrics, select_variant

    states = json.loads(EXPERIMENTS.read_text()) if EXPERIMENTS.exists() else {}
    output = []
    for variant, label in VARIANTS.items():
        state = states.get(variant)
        if state:
            # Process the frozen signal before selecting the next portfolio.
            advance_observed(state, frames, latest, state["pending"]["selected"], market)
        held = {p["ticker"] for p in state["positions"]} if state else set()
        selected = select_variant(screen, market["active"], variant, held)
        if state is None:
            state = new_paper_portfolio(latest, selected, market)
            state["signal_log"] = [dict(state["pending"])]
            states[variant] = state
        elif state.get("selection_session") != latest.date().isoformat():
            # Replace only the newly generated signal, never one from an earlier run.
            state["pending"] = observed_signal(latest, selected, market)
            state["signal_log"][-1] = dict(state["pending"])
        state["selection_session"] = latest.date().isoformat()
        output.append({"id": variant, "label": label, **metrics(state)})
    write_json(EXPERIMENTS, states)
    return output
...
def run() -> dict[str, Any]:
    ...
    for path in (PAPER_PORTFOLIO, EXPERIMENTS):
        if path.exists():
            ...
            portfolios = saved.values() if path == EXPERIMENTS else [saved]
            for portfolio in portfolios:
                tickers.extend(p["ticker"] for p in portfolio.get("positions", []))
                tickers.extend(p["ticker"] for p in portfolio.get("pending", {}).get("selected", []))
    ...
    payload = {
        ...
        "experiments": update_experiments(frames, latest, screen, benchmark),
        "methodology": {
            "formulas": [
                "SMAₙ(t) = Σ P[t−k] / n",
            ...
            "limitations": [
                "Signals are timestamped and frozen. Missing refresh days do not create retrospective recommendations; the last holdings are valued without new trades.",
